...
<div className="mt-4 grid grid-cols-3 gap-2 pt-4 border-t">
  ...
  {/* LINKER TEIL: Der schmale Fit-Out Balken (wie in 'alt') */}
  ...
  {/* RECHTER TEIL: Das Haupt-Chart (Bars/Waterfall) */}
  <div className="h-48 col-span-2">
    <div className="flex justify-end gap-2 mb-1">
      <button 
        onClick={() => setViewMode("bars")} 
        className={`text-[10px] px-1 border rounded ${viewMode === 'bars' ? 'bg-gray-200 font-bold' : ''}`}
      >
        Bars
      </button>
      <button 
        onClick={() => setViewMode("waterfall")} 
        className={`text-[10px] px-1 border rounded ${viewMode === 'waterfall' ? 'bg-gray-200 font-bold' : ''}`}
      >
        Waterfall
      </button>
    </div>
